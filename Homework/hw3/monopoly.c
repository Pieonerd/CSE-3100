...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
	for(int i = 0; i < m; i++)
	{
		...
		if((prop[p[i].loc].owner_id == -1) && (prop[p[i].loc].id != 0)) {
            prop[p[i].loc].owner_id = p[i].id;
        }

		if(prop[p[i].loc].owner_id != p[i].id) {
			if((p[i].balance < prop[p[i].loc].rent) && (prop[p[i].loc].id != 0)) {
				transaction(&p[i], &p[prop[p[i].loc].owner_id], prop[p[i].loc].rent);
				return 0;
			} else {
				transaction(&p[i], &p[prop[p[i].loc].owner_id], prop[p[i].loc].rent);
			}
