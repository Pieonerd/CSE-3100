...
void * thread_main(void * arg_in)
{
    ...
    char max_value_msg[MSG_BUF_SIZE];
    snprintf(max_value_msg, MSG_BUF_SIZE, "%d\n", gmn_get_max());
    int rv = send_str(sockfd, max_value_msg);
    if(rv < 0) {
        thread_die(arg);
    }
    ...
    while(1) {
        ...
        rv = recv_lines(sockfd, str, MSG_BUF_SIZE);
        if(rv < 0) {
            thread_die(arg);
        }
        ...
        snprintf(str, MSG_BUF_SIZE, "%d\n", result);
        rv = send_str(sockfd, str);
        if(rv < 0) {
            thread_die(arg);
        }

        if(result == 0) {
            char *final_msg = gmn_get_message(&gmn);
            rv = send_str(sockfd, final_msg);
            if(rv < 0) {
                thread_die(arg);
            }
            break;
